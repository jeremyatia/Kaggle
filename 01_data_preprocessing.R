...
Summary_Driver_Trips <- function(driver) {
  ...
  t=1 # trip counter (from 1 to 200)
    ...
    #Create some variables for each record of the trip
		...
		q1_speed=quantile(speed, probs = 0.25); q2_speed=quantile(speed, probs = 0.50); q3_speed=quantile(speed, probs = 0.75)
		#Acceleration
		avg_acc=mean(acc); sd_acc=sd(acc); min_acc=min(acc); max_acc=max(acc)
		q1_acc=quantile(acc, probs = 0.25); q2_acc=quantile(acc, probs = 0.50); q3_acc=quantile(acc, probs = 0.75)
		#Distance to origin
		avg_dist_to_orig=mean(dist_to_origin); sd_dist_to_orig=sd(dist_to_origin); max_dist_to_orig=max(dist_to_origin)
		q1_dist_to_orig=quantile(dist_to_origin, probs = 0.25); q2_dist_to_orig=quantile(dist_to_origin, probs = 0.50); q3_dist_to_orig=quantile(dist_to_origin, probs = 0.75)
		#Others
		trip_duration=max(time);  crow_flight=sqrt(coord[nrow(coord), 1]^2 + coord[nrow(coord), 2]^2)
		acc_time=sum(acc_flag); dec_time=sum(dec_flag)
